...
production url: https://musictrends--api.herokuapp.com/

A rake db:seed call imports data from GENIUS API into a POSTGRES database. Rails, ActiveRecord, and ActiveSerializers are used to build a paginated JSON API which exposes the MusicTrends data analysis, values generated by several different background workers using Redis/Sidekiq.
...
Note: 
* http://localhost:3000 is the URL of the locally launched project.
* https://musictrends--api.herokuapp.com is the url of the production level api
...
```
`/songsearch` -> mandatory_params: {query: "query string"}
`/songinfo/:song_id` -> mandatory_params: {search: {text: "query string"}}

GET `/songs` -> possible_params: {page: n}
GET `/songs/:id`
GET `/searches` -> possible_params: {page: n}
GET `/searches/:id`
GET `/tags` -> possible_params {page: n}
GET `/tags/:id`
POST `/tags` -> mandatory_params: {tags: {context: 'string', key_words: ["key1", "key2"]}, page: n}
PUT/PATCH `/tags/:id` -> possible_params: {tags: {context: 'string', key_words: ["key1", "key2"]}}
DELETE `/tags/:id`
```
...
`POST `https://musictrends--api.herokuapp.com/tags/``
